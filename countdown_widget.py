...
"""
╔══════════════════════════════════════════════════════════╗
║  WorkCountdown v1.3 — Виджет рабочего дня               ║
║  Windows 10/11 | Python 3.8+                            ║
╚══════════════════════════════════════════════════════════╝
Changelog v1.3:
  • FIX  Настройки больше не крашат приложение (Toplevel bug)
  • FIX  Колесо цвета: правильная привязка клика к тону
  • FIX  Resize: кнопка ◢ больше не прячется
  • FIX  Minimalist: края цифр не обрезаются
  • NEW  Маска ЧЧ:ММ в полях времени (автодвоеточие)
  • NEW  Процент / прогресс-бар в минималистичном режиме
"""
...
TKEY     = "#010101"   # прозрачный ключ (minimalist)
...
DEF_CFG = {
    ...
    "color_timer":   None,
    "minimalist":    False,
    "mini_show_pct": True,   # показывать % и бар в минималистичном режиме
}
...
def get_theme(cfg):
    ...
    t["bar_fg"] = t["accent"]
    if cfg.get("color_bg"):
        t["bg"] = cfg["color_bg"]; t["hdr"] = cfg["color_bg"]
    if cfg.get("color_timer"):
        t["accent"] = cfg["color_timer"]; t["bar_fg"] = cfg["color_timer"]
    return t
...
class TimeEntry(ttk.Entry):
    """
    Entry с автоматической маской ЧЧ:ММ.
    - Принимает только цифры
    - Автовставляет ':' после 2 цифр
    - Нормализует ЧЧ ≤ 23, ММ ≤ 59 при потере фокуса
    """
    ...
    def _fmt(self, e=None):
        if self._lock: return
        key = getattr(e, "keysym", "")
        # Навигационные клавиши не трогаем
        if key in ("BackSpace","Delete","Left","Right","Tab","Home","End"): return
        self._lock = True
        try:
            raw = self._var.get()
            ...
            if len(digits) >= 3:
                ...
                result = digits + ":"
                pos = 3          # курсор прыгает за ":"
            else:
                result = digits
            if result != raw:
                self._var.set(result)
                self.icursor(min(pos, len(result)))
        finally:
            self._lock = False
    ...
    def _normalize(self, e=None):
        ...
        self._lock = True
        try:
            digits = "".join(c for c in self._var.get() if c.isdigit())
            h = min(int(digits[:2]), 23) if len(digits) >= 2 else 0
            m = min(int(digits[2:4]), 59) if len(digits) >= 4 else 0
            self._var.set(f"{h:02d}:{m:02d}")
        except: pass
...
class Ptcl:
    ...
    def __init__(self, x, y, col):
        ...
        self.dec = random.uniform(0.010, 0.028); self.sz = random.uniform(2, 5)
...
class FWWindow(tk.Toplevel):
    ...
    def __init__(self, parent, mx=0, my=0, mw=None, mh=None):
        ...
        self.configure(bg="black"); self.attributes("-alpha", 0.93)
    ...
    def _frame(self):
        ...
        for p in self.pts:
            ...
            cv.create_oval(p.x-sz, p.y-sz, p.x+sz, p.y+sz,
                           fill=f"#{r:02x}{g:02x}{b:02x}", outline="", tags="p")
...
class ColorPickerDialog(tk.Toplevel):
    """
    HSV colour picker:
     - Кольцо оттенков (360 секторов), красный сверху, по часовой стрелке
     - SV-квадрат (22×22 ячейки)
    ...
    def _draw_wheel(self):
        ...
        for i in range(360):
            ...
            # Красный (hue=0) сверху (90°), сектора по часовой стрелке
        ...
        # hue=0 → вверх (90° в tkinter), hue=0.25 → вправо (0°), ...
        ang = math.radians(90 - self._h * 360)
        mid = (ri + ro) / 2
        mx = cx + mid * math.cos(ang)
        my = cy - mid * math.sin(ang)  # минус: y растёт вниз
    ...
    def _whl_event(self, e):
        ...
        if dist < self.WHL_IN - 8 or dist > self.WHL_OUT + 10:
            return
        # ─── FIX: правильная формула клик→тон ───────────────────────────────
        # atan2(dy, dx) в экранных координатах (y↓):
        #   вверх (dy<0, dx≈0) → -90°, вправо → 0°, вниз → +90°, влево → ±180°
        # Нам нужно: вверх=hue 0, по часовой → hue растёт
        # ∴ hue = (atan2_degrees + 90) % 360 / 360
        angle = math.degrees(math.atan2(dy, dx))
        self._h = ((angle + 90) % 360) / 360
        self._draw_wheel(); self._draw_sv(); self._update_preview()
    ...
    def _draw_sv(self):
        ...
        for si in range(N):
            for vi in range(N):
                r, g, b = colorsys.hsv_to_rgb(self._h, si/N, 1.0 - vi/N)
                col = f"#{int(r*255):02x}{int(g*255):02x}{int(b*255):02x}"
                x1, y1 = si*cw, vi*ch
...
class SettingsWin(tk.Toplevel):
    ...
    def _time_row(self, p, lbl, var):
        """Строка с маскированным вводом ЧЧ:ММ."""
        f = tk.Frame(p, bg=self.t["bg"]); f.pack(fill=tk.X, padx=10, pady=3)
        tk.Label(f, text=lbl, bg=self.t["bg"], fg=self.t["dim"],
                 font=("Segoe UI",9), anchor="w").pack(side=tk.LEFT)
        TimeEntry(f, textvariable=var).pack(side=tk.RIGHT)
    ...
    def _color_row(self, parent, label, cfg_key):
        ...
        def _on_pick(h): self.cfg[cfg_key] = h; swatch.config(bg=h)
        def _reset():
            self.cfg[cfg_key] = None
            swatch.config(bg=THEMES[self.cfg.get("theme","dark")]["bg"]
                          if cfg_key=="color_bg"
                          else THEMES[self.cfg.get("theme","dark")]["accent"])
    ...
    def _build(self):
        ...
        self._time_row(lf1, "Начало рабочего дня (пн–вс):", self.v_start)
        self._time_row(lf1, "Конец рабочего дня (пн–чт):",  self.v_wd)
        self._time_row(lf1, "Конец рабочего дня (пятница):", self.v_fr)
        tk.Label(lf1, bg=t["bg"], fg=t["dim"], font=("Segoe UI",8),
                 text="  Время московское. Формат ЧЧ:ММ — вводите только цифры").pack(
        ...
        self._time_row(lf2, "Начало сегодня:", self.v_cs)
        self._time_row(lf2, "Конец сегодня:",  self.v_ce)
        tk.Label(lf2, bg=t["bg"], fg=t["dim"], font=("Segoe UI",8),
                 text="  Пример: введи 0935 → автоматически станет 09:35").pack(
                 anchor="w", padx=6, pady=(0,5))
        ...
        self.v_mini_pct = tk.BooleanVar(value=self.cfg.get("mini_show_pct", True))
        ...
        # Сублопция: показывать % в мини-режиме
        pct_f = tk.Frame(lf4, bg=t["bg"]); pct_f.pack(fill=tk.X, padx=30, pady=(0,2))
        ttk.Checkbutton(pct_f, text="└─ показывать прогресс и % в мини-режиме",
                        variable=self.v_mini_pct).pack(anchor="w")
    ...
    def _save(self):
        for var, name in [(self.v_start,"Начало"),(self.v_wd,"Конец Пн–Чт"),(self.v_fr,"Конец Пт")]:
            if not valid_t(var.get()):
                messagebox.showerror("Ошибка",
                    f"Неверный формат «{name}»: {var.get()!r}\nФормат: ЧЧ:ММ", parent=self)
                return
        today = datetime.datetime.now().strftime("%Y-%m-%d")
        ...
        self.cfg.update(
            ...
            mini_show_pct = self.v_mini_pct.get(),
        )
        autostart_set(self.v_auto.get())
        save_cfg(self.cfg)
        self.on_save(self.cfg)  # применяем к виджету (он НЕ уничтожает это окно)
        self.destroy()          # закрываем настройки
...
class Widget(tk.Tk):
    ...
    def _ui(self):
        # ─── FIX: уничтожаем только НЕ-Toplevel детей ──────────────────────
        # Если уничтожить Toplevel (SettingsWin), то _save() упадёт с TclError
        # и приложение закроется. Поэтому Toplevel-ы не трогаем.
        for w in list(self.winfo_children()):
            if not isinstance(w, tk.Toplevel):
                w.destroy()

        # Снимаем прозрачный ключ при выходе из мини-режима
        if WINDOWS:
            try: self.attributes("-transparentcolor", "")
            except: pass

        if self.cfg.get("minimalist", False):
            self._ui_minimalist(); return
        ...
        # Тело — ключевое: grip всегда в самом низу wrap, не inside body
        body  = tk.Frame(wrap, bg=t["bg"]); body.pack(fill=tk.BOTH, expand=True)
        inner = tk.Frame(body, bg=t["bg"]); inner.pack(padx=16, pady=(10,0), fill=tk.X)
        ...
        self._ls.pack(fill=tk.X, pady=(0,6))

        # ─── FIX resize grip: помещаем в wrap последним → всегда виден ──────
        grip_row = tk.Frame(wrap, bg=t["bg"], height=14)
        grip_row.pack(fill=tk.X, side=tk.BOTTOM); grip_row.pack_propagate(False)
        self._grip = tk.Label(grip_row, text="◢", bg=t["bg"], fg=t["border"],
    ...
    def _ui_minimalist(self):
        """
        Полностью прозрачный фон, только плавающий таймер.
        ─── FIX: canvas шире текста (+ отступ для теней) ────────────────────
        """
        W  = self._W
        fz = max(28, min(96, W // 5))

        # Вычисляем нужную ширину: символ "0" в Segoe UI ≈ 0.65*fz, 8 символов
        text_needed = int(fz * 0.68 * 8) + 28   # 28px: тени + поля
        W = max(W, text_needed)                   # расширяем если надо

        show_pct = self.cfg.get("mini_show_pct", True)
        timer_h  = fz * 2
        extra_h  = 28 if show_pct else 0         # бар + процент
        H = timer_h + extra_h + 14
        ...
        self._mini_H      = H
        self._mini_fz     = fz
        self._mini_timer_y = H // 2 - (extra_h // 2)
        self._mini_bar_y   = self._mini_timer_y + fz + 4
        self._mini_pct_y   = self._mini_bar_y + 14
        self.geometry(f"{W}x{H}")
        self._t = get_theme(self.cfg)
        ...
        self._draw_mini("00:00:00", "#888888", 0.0)

    def _draw_mini(self, text, color, progress=0.0):
        """Рисуем таймер + (опционально) прогресс-бар и % на канвасе."""
        cv  = self._mini_cv
        W   = self._mini_W
        cx  = W // 2
        cy  = self._mini_timer_y
        fnt = ("Segoe UI", self._mini_fz, "bold")
        show_pct = self.cfg.get("mini_show_pct", True)

        cv.delete("shadow"); cv.delete("timer_text")
        cv.delete("bar");    cv.delete("pct_text")

        # Тень (чёрные сдвинутые копии)
        ...
        if show_pct:
            ...
            bar_y = self._mini_bar_y
            # Фоновая полоска
            cv.create_rectangle(bar_x0, bar_y, bar_x1, bar_y+5,
                                 fill="#1a1a3a", outline="", tags="bar")
            # Заполненная часть
            if progress > 0:
                fw = max(2, int((bar_x1 - bar_x0) * progress))
                cv.create_rectangle(bar_x0, bar_y, bar_x0+fw, bar_y+5,
                                     fill=color, outline="", tags="bar")
            # Процент
            pct_str = f"{int(progress * 100)}%"
            cv.create_text(cx, self._mini_pct_y, text=pct_str,
                           font=("Segoe UI", 9), fill=color, tags="pct_text")
    ...
    def _rs_stop(self, e):
        """
        ─── FIX: НЕ пересоздаём весь UI — grip остаётся виден ──────────────
        Просто пересчитываем шрифт под новую ширину.
        Прогресс-бар и метки уже используют fill=X и адаптируются сами.
        """
        w = self.winfo_width()
        self.cfg["win_w"] = w; self._W = w
        save_cfg(self.cfg)
        fz = max(22, min(84, w // 6))
        try:
            self._lt.config(font=("Segoe UI", fz, "bold"))
        except tk.TclError:
            pass
    ...
    def _apply(self, cfg):
        """
        Применяем настройки к живому виджету.
        ─── FIX: не вызываем quit/destroy — приложение продолжает работать ──
        """
        ...
        self._ui()   # перестраиваем UI с новыми настройками
    ...
    def _draw_bar(self, progress):
        ...
        try:
            ...
            self._bar.delete("b")
            if progress > 0:
                self._bar.create_rectangle(0, 0, max(4, int(bw*progress)), 6,
    ...
    def _tick(self):
        ...
        # Цвет таймера
        if   status == "before":  t_color = t["dim"]
        elif status == "working": t_color = (t["warn"] if now.second%2==0 else t["ok"]) \
                                             if rem < 1800 else t["accent"]
        else:                     t_color = t["ok"]

        timer_text = fmt(rem) if status != "done" else "00:00:00"

        if is_mini:
            try: self._draw_mini(timer_text, t_color, prog)
            except tk.TclError: pass
        else:
            try:
                if status == "before":
